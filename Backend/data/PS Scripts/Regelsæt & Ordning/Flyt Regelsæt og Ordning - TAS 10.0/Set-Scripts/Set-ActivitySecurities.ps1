...
if($testPath){
    ...
    foreach($sp in $security_permissions){
        ...
        if($sp.HideFormActivity -eq 'True'){
            $hideFromActivity = 1
        }else{
            $hideFromActivity = 0
        }
        ...
        if($doesNameExist){
            ...
            if($exist -eq $false){
                ...
                $insert = $insert + "($id_sp,'$name_sp','$status_sp',$synlig_sp, $hideFromActivity), `r`n"   

                if($security_permissions.Count -gt 100){
                    if($i -eq 100){
                        if(!([string]::IsNullOrEmpty($insert))){
                            write-host "The INSERT query has been created.`r`n"
                            $insert = "INSERT INTO AKTIVITET_SECURITY(AKTIVITET_ID,NAME,STATUS,synlig, HideFromActivity) `r`n" + 'VALUES' + $insert
    ...
    if(!([string]::IsNullOrEmpty($insert))){
        ...
        $insert = "INSERT INTO AKTIVITET_SECURITY(AKTIVITET_ID,NAME,STATUS,synlig, HideFormActivity) `r`n" + 'VALUES' + $insert
        $insert = $insert.Substring(0,$insert.Length-4) #remove the last ','
