...
if($ordning_id -ne $false){
    ...
    if($testPath){
        ...
        foreach($s in $schemaSettings_csv){
            ...
            $data_sync_CaseLogState = isNull($s.CaseLogState)
            ...
            if($data_sync_NextActivity -ne ''){
                ...
                $data_sync_NextActivity = isNull_int($data_sync_NextActivity)
            }
            
            if(([string]::IsNullOrEmpty($s_luk))){
                $s_luk = isNull($s_luk)
            }
            ...
            if($isActivityLinkedToOrdning){
                ...
                if($doesSchemaExist){
                    if(!([string]::IsNullOrEmpty($s_åben))){
                        ...
                        if($s_luk -ne 'NULL'){
                            ...
                            $newCloseActivityId = $s_luk
                        }
                                               
                        if($s_tjekliste_id -ne 'NULL'){
                            if($schema_object -eq 'DynamicFrontSystem'){
                               
                                $DoesSchemaSettingExist = DoesSchemaSettingExist -FORSYSTEM_NAME $s_forsystem_name -OPEN_ACTIVITY $newOpenActivityId -CLOSE_ACTIVITY $newCloseActivityId -PARM $schema_param -OBJEKT $schema_object -TjeklisteId $s_tjekliste_id
                            }else{
                                
                                $DoesSchemaSettingExist = DoesSchemaSettingExist -FORSYSTEM_NAME $s_forsystem_name -OPEN_ACTIVITY $newOpenActivityId -CLOSE_ACTIVITY $newCloseActivityId -PARM $schema_param -OBJEKT $schema_object -NextActivityId $data_sync_NextActivity -ApproveActivity $data_sync_ApproveActivity -NumberOfRespiteDays $data_sync_NumberOfRespiteDays -CaseLogState $data_sync_CaseLogState -ActionOnCompletion $data_sync_ActionOnCompletion -TjeklisteId $s_tjekliste_id
                            }
                        
                        }else{
                            if($schema_object -eq 'DynamicFrontSystem'){
                                $DoesSchemaSettingExist = DoesSchemaSettingExist -FORSYSTEM_NAME $s_forsystem_name -OPEN_ACTIVITY $newOpenActivityId -CLOSE_ACTIVITY $newCloseActivityId -PARM $schema_param -OBJEKT $schema_object
                            }else{
                                $DoesSchemaSettingExist = DoesSchemaSettingExist -FORSYSTEM_NAME $s_forsystem_name -OPEN_ACTIVITY $newOpenActivityId -CLOSE_ACTIVITY $newCloseActivityId -PARM $schema_param -OBJEKT $schema_object -NextActivityId $data_sync_NextActivity -ApproveActivity $data_sync_ApproveActivity -NumberOfRespiteDays $data_sync_NumberOfRespiteDays -CaseLogState $data_sync_CaseLogState -ActionOnCompletion $data_sync_ActionOnCompletion
                            }
                        ...
                        if(!$DoesSchemaSettingExist){
                            ...
                            if(([string]::IsNullOrEmpty($newCloseActivityId))){
                                ...
                                if($s_tjekliste_id -ne 'NULL'){
                                    $tjekliste_data_id = GetTjeklisteDataId -TjeklisteId $s_tjekliste_id
                                    $insert_schema_setting = "INSERT INTO ORDNING_FORSYSTEM(FORSYSTEM_ID, ORDNING,AKTIVITET_ID,TIMESTAMP,LUK_AKTIVITET_ID,STATISTIK_MODUL_ID,TJEKLISTEDATA_ID, STATISTIK_MODUL_ID_BRUGER, TJEKLISTEDATA_TJEKLISTEID, GENAABEN_AKTIVITET_ID, OBJEKT, PROJEKTTYPE, BUDGET_OMRAADE_ID, INDSATSOMRAADE_ID, IsPublic) `r`n VALUES($newSchemaId, '$ordning',$newOpenActivityId, '$timeStamp',$newCloseActivityId,NULL,$tjekliste_data_id,NULL,$s_tjekliste_id ,$s_genåben,NULL,NULL,NULL,NULL, $s_public_int) `r`n"
                                }else{
