...
if($ordning_id -ne $false){
    Title -Title 'Get-Sagsbemyndiget'
    try{
        ...
        if($connection.State -eq "Open"){cls
            ...
            Write-Host `r`n Retrieved $result.Count Sagsbemyndiget: 
            foreach($item in $result){
                Write-Host $item.INITIALER - $item.BUDGET_OMRAADE_ID -ForegroundColor Green
            } 

            $result | Export-Csv -Path $path\Sagsbemyndiget.csv -NoTypeInformation -Encoding UTF8
