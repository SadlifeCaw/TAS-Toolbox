...
try{
    ...
    if($connection.State -eq "Open"){
        $query = "select distinct ORDNING
		            ...
		            from REGELSAET
		            where TEKST IN ($list_regelsaet))) AND ORDNING NOT IN('MASTER2')"
...
foreach($single in $SpecificSchemaSettings_csv){
    ...
    foreach($o in $list_ordning){
        ...
        if($isActivityBoundToOrdningOpen -and $isActivityBoundToOrdningClose){
            ...
            if(!$doesSchemaSettingExist){
                ...
                try{
                    ...
                    if($connection.State -eq "Open"){
                        ...
                        if($single.PARM -eq 'NULL'){
                            if($single.OBJEKT -eq 'NULL'){
                        ...
                        }elseif($single.PARM.Contains('#CREATEVALIDATION')){
                        ...
                        }elseif($single.OBJEKT -eq 'nv_DataSync'){
                        ...
                        if($single.OBJEKT -eq 'nv_DataSync'){
            ...
            Write-Host Ordning - $o -ForegroundColor Yellow
            Write-Host
        }
    }
}
