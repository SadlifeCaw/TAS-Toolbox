...
if($regelsæt_id -ne $false){
    ...
    try{
        ...
        if($connection.State -eq "Open"){
            ...
            $activities_regel | foreach {
                $_.AKTIVITET_KODE = $_.AKTIVITET_KODE.replace($InitialFra,$InitialTil)
                $_.STATUS_FRA = $_.STATUS_FRA.replace($InitialFra,$InitialTil)
                
                if(!([string]::IsNullOrEmpty($_.STATUS_TIL))){
                    $_.STATUS_TIL = $_.STATUS_TIL.replace($InitialFra,$InitialTil)
                }
                                
                if(!([string]::IsNullOrEmpty($_.AKTIVITET_FORTRYD_ID))){
                    $_.AKTIVITET_FORTRYD_ID = $_.AKTIVITET_FORTRYD_ID.replace($InitialFra,$InitialTil)
                }
            }
