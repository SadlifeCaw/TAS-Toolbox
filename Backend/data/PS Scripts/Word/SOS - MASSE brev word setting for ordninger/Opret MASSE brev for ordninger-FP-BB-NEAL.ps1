...
$ordning = 'MASTER2' #Specify which 'Ordning' to retrive the word setting from.
            ...
            foreach($o in $ordnings){
                ...
                if($skipOrdnings -notcontains $o ){
                    ...
                    if($isActivityLinkedToOrdning){
                        ...
                        ## Change widget settings/filters. 

                        $ords = @()

                        if($skipOrdnings.Count -eq 0){

                            $ords = $ordnings -join ';'

                        }else{
                            foreach ($i in $ordnings){

                                if(!$skipOrdnings.Contains($i)){
                                    $ords += $i
                        ...
                        $widgets = "UPDATE WidgetFilters
                                SET Parameter = '$ords'
                                where WidgetSettingsId IN(
                                    select Id
                                    from WidgetSettings
                                    where title = 'Masseudsendelse af brev'
                                )
	                                AND Field = 'SchemeId'"
                        
                        if($widgets -ne ''){
                            if($connection.State -eq 'Open'){
                                Write-Host widgets have been updated -ForegroundColor green 
                                $result = Invoke-Sqlcmd -Query $widgets -ConnectionString $connectionstring -MaxCharLength $maxCharLength
